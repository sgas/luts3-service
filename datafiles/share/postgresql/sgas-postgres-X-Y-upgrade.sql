...
DROP FUNCTION IF EXISTS urcreate;
...
CREATE OR REPLACE FUNCTION urcreate (
    ...
    in_extra_resource_type     text[],
    in_extra_resource_usage_type  text[],
    in_extra_resource_amount   float[],
    in_extra_resource_unit     text[]
)
...
BEGIN
    ...
    IF in_extra_resource_type IS NOT NULL THEN
        FOR i IN array_lower(in_extra_resource_type, 1) .. array_upper(in_extra_resource_type, 1) LOOP
            -- Check if the resource_type exists
            SELECT INTO resource_type_id id FROM resource_type WHERE resource_type = in_extra_resource_type[i];
            IF NOT FOUND THEN
                INSERT INTO resource_type (resource_type) VALUES (in_extra_resource_type[i]) RETURNING id INTO resource_type_id;
            END IF;
            -- insert extra_resources
            INSERT INTO extra_resources (usagedata_id, resource_type_id, usage_type, amount, unit)
                   VALUES (ur_id, resource_type_id, in_extra_resource_usage_type[i]::resource_usage_type, in_extra_resource_amount[i], in_extra_resource_unit[i]);
        END LOOP;
    END IF;
