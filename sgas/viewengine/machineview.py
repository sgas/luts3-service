...
class MachineView(baseview.BaseView):
    ...
    def retrieveMachineInfo(self):
        ...
        defs = []
        for query in [ QUERY_EXECUTED_JOBS_PER_DAY, QUERY_TOP10_USERS ]:
    ...
    def renderMachineView(self, results, request):
        ...
        dict_elements = []
        dict_elements += [ ((u_batches[0], u[0]), u[1]) for u in top_users ]
        dict_elements += [ ((u_batches[1], u[0]), u[2]) for u in top_users ]

        u_matrix = dict(dict_elements)
        ...
        request.write('<h5>Top users for the last month</h5>\n')
