...
def get_hostscalefactor_type(db_cur, scalefactor_type_id: int = None) -> str:
    if scalefactor_type_id is None:
        query = """
            select
                factor_type
            from hostscalefactor_types
            where id = (select hostscalefactor_type_default.id FROM hostscalefactor_type_default LIMIT 1)
        """
        args: tuple = ()
    else:
        query = """
            ...
            where id = %s
        """
        args = (scalefactor_type_id,)

    execute_sql(db_cur, query, tuple(args))
    row = db_cur.fetchone()
    return row[0]
...
def show_factors(db, machine_name, time, factor_type, def_ids):
    ...
    if factor_type is not None:
        ...
        factor_type = get_hostscalefactor_type(cur)
    ...
    print("%-40s  %-45s  %-10s %s" % ("Machine", "Validity period", factor_type, "Definition id"))
    print("-----------------------------------------------------------------------------------------------------------------")
    for row in cur.fetchall():
        ...
        factor = float(row[3])
        print("%-40s  %s  %9.5f  %3i" % (machine_name, validity_period, factor, hs_id))


def add_factor(db, machine_name, timerange, factortype, factor, add_machine, add_factor_type):
    ...
    if factortype not in factor_types:
        if add_factor_type:
            print(f"Factor type {factortype} unknown - adding to the db")
            execute_sql(cur,"insert into hostscalefactor_types (factor_type) values (%s);", (factortype,))
            factor_types = get_factor_types(cur)
        else:
...
def main():
    ...
    argparser.add_argument('-A', '--add_factor_type', action='store_true', help="Add factor type to db if unknown when adding a factor")
    ...
    if args.action == 'show':
        ...
        try:
            add_factor(db, args.machine_name, timerange, args.factortype, args.factor or '', args.add_machine, args.add_factor_type)
